...
__global__ void dtw_kernel(double *x, int xsize, double *y, int ysize, double *distance) {
    ...
    __shared__ double smem[90];
    __shared__ double x_s[8];
    __shared__ double y_s[8];
...
int main() {
    ...
    double y[] = {10.648, 30.333, 53.143, 72.429, 83.952, 86.571, 80.095, 65};

    int ysize = sizeof(y) / sizeof(double);

    for (int i=0; i < (o_xsize-8+1); i++) {
	    double x[8]={0};
	    int l = i;
	    for (int k=0; k<8; k++) {
		    //printf("%f ",  o_x[l]);
