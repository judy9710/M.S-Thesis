...
__global__ void generateMatrix(double* d_matrices, int num_matrices, double *y, int ysize, double *temp) {
    ...
    if (tid< num_matrices){
	    ...
	    d_matrices[tid*100]=first;
	    // fill in the first row
	    for (int i=1; i<10; i++){
		    double diff = fabs(temp[tid*10+i]-y[0]);
		    d_matrices[i+100*tid]=diff+d_matrices[i+100*tid-1];
	    ...
	    for (int i=10; i<100; i+=10){
		    ...
		    d_matrices[i+100*tid]=diff+d_matrices[i+100*tid-10];
		    //printf("%lf ", d_matrices[i]);
	    }
	    //printf("\n");
	    ...
	    for (int i=1; i<10; i++){
		    for (int j=1; j<10; j++){
			    double min_cost;
			    min_cost=fmin(d_matrices[i*10+j-1+tid*100], d_matrices[i*10+j-10+tid*100]);
			    min_cost=fmin(d_matrices[i*10+j-11+tid*100], min_cost);
			    d_matrices[i*10+j+tid*100]=fabs(temp[tid*10+j]-y[i])+min_cost;
...
int main() {
    ...
    //double o_x[] = {1.0, 7.0, 3.0, 4.0, 1.0, 10.0, 5.0, 4.0, 7.0, 4.0, 3.0, 7.0, 5.0, 4.0, 5.0, 7.0, 5.0, 2.0, 3.0, 9.0};
     //double x[] = {1.0, 7.0, 3.0, 4.0, 1.0, 10.0, 5.0, 4.0, 7.0, 4.0};
    //int o_xsize = sizeof(o_x) / sizeof(double); 
    //double y[] = {1.0, 4.0, 5.0, 10.0, 9.0, 3.0, 2.0, 6.0, 8.0, 4.0};
    //int ysize = sizeof(y) / sizeof(double);
