...
__global__ void dtw_kernel(double *x, int xsize, double *y, int ysize, double *distance) {
    ...
    __shared__ double smem[1088];
    __shared__ double x_s[31];
    __shared__ double y_s[31];
    ...
    int new_ysize = ysize+3; // ysize now becomes 34
...
double dtw(double *x, int xsize, double *y, int ysize) {
    ...
    cudaMemcpy(result, &distance[xsize * (ysize+3) + ysize], sizeof(double), cudaMemcpyDeviceToHost);
...
int main() {
    ...
    double y[] = {10.648, 30.333, 53.143, 72.429, 83.952, 86.571, 80.095, 65, 44.905, 21.762, 2.1129, -10.226, -14.627, -13.883, -11.345, -10.661, -10.318, -10.424, -10.43, -10.377,-10.498,-10.687,-10.767,-10.571,-9.7884,-8.9894,-8.1481,-7.3651,-6.6984,-6.1587,-5.7196};

    int ysize = sizeof(y) / sizeof(double);

    for (int i=0; i < (o_xsize-31+1); i++) {
	    double x[31]={0};
	    int l = i;
	    for (int k=0; k<31; k++) {
		    //printf("%f ",  o_x[l]);
