...
__global__ void dtw_kernel(double *x, int xsize, double *y, int ysize, double *distance) {
    ...
    __shared__ double smem[380];
    __shared__ double x_s[18];
    __shared__ double y_s[18];
...
int main() {
    ...
    double y[] = {10.648, 30.333, 53.143, 72.429, 83.952, 86.571, 80.095, 65, 44.905, 21.762, 2.1129, -10.226, -14.627, -13.883, -11.345, -10.661, -10.318, -10.424};

    int ysize = sizeof(y) / sizeof(double);

    for (int i=0; i < (o_xsize-18+1); i++) {
	    double x[18]={0};
	    int l = i;
	    for (int k=0; k<18; k++) {
		    //printf("%f ",  o_x[l]);
