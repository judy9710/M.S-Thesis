...
__global__ void dtw_kernel(double *x, int xsize, double *y, int ysize, double *distance) {
    ...
    __shared__ double smem[6084];
    __shared__ double x_s[78];
    __shared__ double y_s[78];
...
int main() {
    ...
    double y[]= {10.648, 30.333, 53.143, 72.429, 83.952, 86.571, 80.095, 65, 44.905, 21.762, 2.1129, -10.226, -14.627, -13.883, -11.345, -10.661, -10.318, -10.424, -10.43, -10.377,-10.498,-10.687,-10.767,-10.571,-9.7884,-8.9894,-8.1481,-7.3651,-6.6984,-6.1587,-5.7196, -5.3492, -5.0106, -4.672, -4.3228, -3.9788
-3.6508, -3.328, -3.0265, -2.7884, -2.6402, -2.5608, -2.5185, -2.5026, -2.4974, -2.4762, -2.4233, -2.3333, -2.1746, -1.8942, -1.4497, -0.8201, 0.0476, 1.2063, 2.672, 4.4127, 6.4021, 8.6138, 10.989, 13.429, 15.847, 18.191, 20.434, 22.566, 24.593, 26.518, 28.339, 30.042, 31.63, 33.079, 34.318, 35.233, 35.73, 35.762, 35.328, 34.439
, 33.116, 31.402};
    int ysize = sizeof(y) / sizeof(double);

    for (int i=0; i < (o_xsize-78+1); i++) {
	    double x[78]={0};
	    int l = i;
	    for (int k=0; k<78; k++) {
		    //printf("%f ",  o_x[l]);
